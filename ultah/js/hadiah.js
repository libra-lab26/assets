...
function multifungsi(){
  ...
  function kalimatakhir(){
    let teks1 = `Ini dia, kado aneh yang paling absurd yang mungkin bisa bikin kamu makin keren, makin bahagia, atau mungkin... makin bingung mau pakai buat apa. 😂 <br> Meskipun kado ini aneh dan tidak berguna, tapi ga ada niat buruk kok, cuma untuk mengurangi beban koleksi ku wkwkwk`;
    let teks2 = `Semoga di usia baru ini, kamu semakin sabar, semakin cantik, semakin manis, dan tentunya semakin ... tambah tua (eh, maksudnya tambah baik dan ga cuek :v). <br>Semoga ini menjadi hari ulang tahun terbaik dalam hidupmu! 💫 <br> <br><span class='ft'>Wish You All The Best ✨</span>`;
    new TypeIt("#kalimat", {
      strings: [teks1], 
      startDelay: 50, 
      speed: 37, 
      ...
      afterComplete: function(){
        setTimeout(function(){
          kalimat.innerHTML = ""; // Hapus teks pertama setelah delay
          new TypeIt("#kalimat", {
            strings: [teks2],
            startDelay: 50,
            speed: 37,
            cursor: true,
            afterComplete: function(){
              kalimat.innerHTML = teks2;
              munculteksnim(); // Memunculkan teksnim setelah teks 2 muncul
            }
          }).go();
        }, 2000); // Delay sebelum teks kedua muncul
      }
    }).go();
  }
  ...
  function munculteksnim(){
    ...
    let teksnim = document.getElementById("teksnim");
    tombol1.insertAdjacentElement("afterend", teksnim);
    teksnim.style="position:relative;opacity:1;transform:scale(1);margin:20px auto";
    setTimeout(jjteksnim, 550);
  }
  
  function jjteksnim(){
    document.getElementById("teksnim").style.animation="rto .8s infinite alternate";
  }
